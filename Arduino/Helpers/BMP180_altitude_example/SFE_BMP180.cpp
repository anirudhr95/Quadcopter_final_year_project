...
// length: number of bytes to read
{
	...
	i2c.beginTransmission(BMP180_ADDR);
	i2c.write(values[0]);
	_error = i2c.endTransmission();
	if (_error == 0)
	{
		i2c.requestFrom(BMP180_ADDR);
//		while(i2c.available() != length) ; // wait until bytes are ready
		for(x=0;x<length-1;x++)
		{
			values[x] = i2c.read();
//     Serial.print(values[x]);
		}
   values[x] = i2c.readLast();
